...
###########编码时。用上面几行代码可强制转换成utf-8也可在每次编码的地方用下面的代码
...
class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim, input_dim, hidden_dim, n_layers=1, dropout_p=0.0):
        ...
        self.word_embeds = nn.Embedding(word_size, word_dim)###（32084，64）
        self.pretrain_embeds = nn.Embedding(pretrain_size, pretrain_dim)####（243004，100）
        self.pretrain_embeds.weight = nn.Parameter(pretrain_embeddings, False)#权重
        self.lemma_embeds = nn.Embedding(lemma_size, lemma_dim)####（24529，32）
        self.dropout = nn.Dropout(self.dropout_p)

        self.embeds2input = nn.Linear(word_dim + pretrain_dim + lemma_dim, input_dim)##Linear(in_features=196,out_features=100,bias=true)
        #self.embeds2input = nn.Linear(64      + 100          + 32       , 100      )
        self.tanh = nn.Tanh()
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, outer_mask_pool, rel_mask_pool, var_mask_pool, tags_info, tag_dim, input_dim, feat_dim, encoder_hidden_dim, n_layers=1, dropout_p=0.1):####encoder_hidden_dim256
        ...
        self.tag_dim = tag_dim###########128
        self.input_dim = input_dim###########128
        self.feat_dim = feat_dim###########256
        self.hidden_dim = encoder_hidden_dim * 2###########256*2

        self.n_layers = n_layers###########1
        self.dropout_p = dropout_p###########0.1

        self.dropout = nn.Dropout(self.dropout_p)
        self.tag_embeds = nn.Embedding(self.tags_info.all_tag_size, self.tag_dim)####tag_embeds  Embedding(24739,128)

        self.struct2rel = nn.Linear(self.hidden_dim, self.tag_dim)#####Linear(in_features=512,out_features=128，bias=Ture)
        self.rel2var = nn.Linear(self.hidden_dim, self.tag_dim)#####Linear(in_features=512,out_features=128，bias=Ture)
        ...
        self.lstm = nn.LSTM(self.tag_dim, self.hidden_dim, num_layers= self.n_layers)#input_size=tag_dim=125 hidden_size=512
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 

        self.feat = nn.Linear(self.hidden_dim + self.tag_dim, self.feat_dim)#Linear(in_features=640,out_features=256,bias=True)
        ...
        self.out = nn.Linear(self.feat_dim, self.tag_size)###256,210,bias=Ture

        self.selective_matrix = Variable(torch.randn(1, self.hidden_dim, self.hidden_dim))#shape[1,512,512]
        if use_cuda:
...
#==================================
    ...
    for instance in dev_instances:
        ...
        if use_cuda:
            ...
            dev_sentence_variables[-1].append(Variable(instance[0]).cuda())
            dev_sentence_variables[-1].append(Variable(instance[1]).cuda())
            dev_sentence_variables[-1].append(Variable(instance[2]).cuda())
        else:
            ...
            dev_sentence_variables[-1].append(Variable(instance[0]))
            dev_sentence_variables[-1].append(Variable(instance[1]))
            dev_sentence_variables[-1].append(Variable(instance[2]))
        ...
        if use_cuda:
            ...
            dev_input1_variables.append(Variable(torch.LongTensor([0] + instance[3])).cuda())
            dev_gold1_variables.append(Variable(torch.LongTensor(instance[3] + [1])).cuda())
        else:
            ...
            dev_input1_variables.append(Variable(torch.LongTensor([0] + instance[3])))
            dev_gold1_variables.append(Variable(torch.LongTensor(instance[3] + [1])))
        p = 0
        all_relations = []
        dev_input2_variable = []
        for i in range(len(instance[3])):
            ...
            if idx == 5 or idx == 6:
                ...
                if len(instance[4][p]) == 0:
                    ...
                    dev_input2_variable.append(Variable(torch.LongTensor([x[1] for x in instance[4][p]])).cuda())
                else:
                    # dev_input2_variable.append(Variable(torch.LongTensor([x[1] for x in instance[4][p]]), volatile=True))
                    dev_input2_variable.append(Variable(torch.LongTensor([x[1] for x in instance[4][p]])))
                p += 1
        ...
        if use_cuda:
            ...
            dev_gold2_variables.append(Variable(torch.LongTensor(sel_gen_relations)).cuda())
        else:
            # dev_gold2_variables.append(Variable(torch.LongTensor(sel_gen_relations), volatile=True))
            dev_gold2_variables.append(Variable(torch.LongTensor(sel_gen_relations)))
        assert p == len(instance[4])
        ...
        for i in range(len(instance[5])):
            ...
            if (idx >= 13 and idx < decoder.tags_info.k_rel_start) or idx >= decoder.tags_info.tag_size:
                ...
                if use_cuda:
                    ...
                    dev_input3_variable.append(Variable(torch.LongTensor(instance[6][p])).cuda())
                else:
                    # dev_input3_variable.append(Variable(torch.LongTensor(instance[6][p]), volatile=True))
                    dev_input3_variable.append(Variable(torch.LongTensor(instance[6][p])))
                p += 1
        ...
        if use_cuda:
            ...
            dev_gold3_variables.append(Variable(torch.LongTensor(all_variables)).cuda())
        else:
            # dev_gold3_variables.append(Variable(torch.LongTensor(all_variables), volatile=True))
            dev_gold3_variables.append(Variable(torch.LongTensor(all_variables)))
        assert p == len(instance[6])
        ...
        if use_cuda:
            ...
            dev_mask1_variables.append(Variable(torch.FloatTensor(mask1)).cuda())
            dev_mask2_variables.append(Variable(torch.FloatTensor(mask2)).cuda())
            dev_mask3_variables.append(Variable(torch.FloatTensor(mask3)).cuda())
        else:
            ...
            dev_mask1_variables.append(Variable(torch.FloatTensor(mask1)))
            dev_mask2_variables.append(Variable(torch.FloatTensor(mask2)))
            dev_mask3_variables.append(Variable(torch.FloatTensor(mask3)))
...
#====================================== test
    ...
    for instance in tst_instances:
        ...
        if use_cuda:
            ...
            tst_sentence_variable.append(Variable(instance[0]).cuda())
            tst_sentence_variable.append(Variable(instance[1]).cuda())
            tst_sentence_variable.append(Variable(instance[2]).cuda())
            ...
            tst_sentence_variable.append(Variable(instance[0]))
            tst_sentence_variable.append(Variable(instance[1]))
            tst_sentence_variable.append(Variable(instance[2]))
...
def test(dev_instances, tst_instances, encoder, decoder):
    ...
    for instance in dev_instances:
        ...
        if use_cuda:
            ...
            dev_sentence_variable.append(Variable(instance[0]).cuda())
            dev_sentence_variable.append(Variable(instance[1]).cuda())
            dev_sentence_variable.append(Variable(instance[2]).cuda())
            ...
            dev_sentence_variable.append(Variable(instance[0]))
            dev_sentence_variable.append(Variable(instance[1]))
            dev_sentence_variable.append(Variable(instance[2]))
...
trn_data = readfile(trn_file)#读出来的是个三元组

word_to_ix = {UNK:0}#均为字典
lemma_to_ix = {UNK:0}
ix_to_lemma = [UNK]
ix_to_word = [UNK]
#print(word_to_ix)
for sentence, _, lemmas, tags in trn_data:#sentence读出的三元组中的第一句 _第二句 lemmas读出第二句 tags第三句
    for word in sentence:
        if word not in word_to_ix:#是对数据集中出现的所有单词索引 无重复
            word_to_ix[word] = len(word_to_ix)#word_to_ix一个单词对应一个索引 按顺序 且保留了一个总长度
            ix_to_word.append(word)#ix_to_word一个索引对应一个单词  按顺序 且保留了一个总长度
        #print word
    for lemma in lemmas:
        if lemma not in lemma_to_ix:#同上
            lemma_to_ix[lemma] = len(lemma_to_ix)
            ix_to_lemma.append(lemma)
#############################################
## tags
tags_info = Tag(tag_info_file,tags_info_file, ix_to_lemma)
SOS = tags_info.SOS
EOS = tags_info.EOS
...
pretrain_data = readpretrain(pretrain_file)#pretrain_data具体读了多少行？
for one in pretrain_data:
    pretrain_to_ix[one[0]] = len(pretrain_to_ix)#只存词的索引
    pretrain_embeddings.append([float(a) for a in one[1:]])#存对应词的向量
...
print ("global tag (w/o variables) dict size: ", tags_info.all_tag_size)
...
#encoder = EncoderRNN(32084         , 64                , 243004             , 100                   , torch.FloatTensor(243004)             , 24529           , 32                 , 100      , 256               , n_layers=2, dropout_p=0.1)
...
#attn_decoder = AttnDecoderRNN(outer_mask_pool, rel_mask_pool, var_mask_pool, tags_info, 128   , 128              , 256               , 256                 , n_layers=1, dropout_p=0.1)
###########################################################
...
if len(sys.argv) == 5 and sys.argv[-1] == "test":
    ...
    trainIters(trn_instances, dev_instances, tst_instances, encoder, attn_decoder, print_every=1000, evaluate_every=10000, learning_rate=0.0005)
